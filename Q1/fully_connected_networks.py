...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        N = x.shape[0]  # Number of input examples
        x_flattened = x.reshape(N, -1)  # Reshape input into (N, D) matrix
        out = x_flattened @ w + b  # x * w + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        dx_flattened = dout @ w.T  # Gradient of x_flattened: dout * w^T
        dx = dx_flattened.reshape(x.shape)  # Reshape dx_flattened back to match the shape of x

        dw = x_flattened.T @ dout  # Gradient of w: x_flattened^T * dout
        db = torch.sum(dout, axis=0)  # Gradient of b: Sum of dout along axis 0 (examples)
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class ReLU(object):
    ...
    def forward(x):
        ...
        ###################################################
        # Replace "pass" statement with your code
        out = x * (x > 0)  # Element-wise multiplication with a boolean mask to apply ReLU
        ###################################################
        #                 END OF YOUR CODE                #
        ###################################################
        cache = x
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        out, fc_cache = Linear.forward(x, w, b)
    
        # Apply ReLU activation
        out, relu_cache = ReLU.forward(out)
    
        # Create cache object for backward pass
    ...
    def backward(dout, cache):
        ...
        dx_relu = ReLU.backward(dout, relu_cache)
    
        # Backward pass through linear transform
        dx, dw, db = Linear.backward(dx_relu, (x, w, b))
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    loss = -torch.mean(torch.log(probs[torch.arange(num_samples), y]))
    
    # Compute the gradient
    dx = probs.clone()
    dx[torch.arange(num_samples), y] -= 1
    dx /= num_samples
    ######################################################################
    #                END OF YOUR CODE                                    #
    ######################################################################
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        reg = self.reg
        loss += reg * (torch.sum(self.params['W1'] * self.params['W1']) + torch.sum(self.params['W2'] * self.params['W2']))
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden_layer_outputs[0] = X
        caches = {}

        for i in range(1, self.num_layers):
          W = self.params[f'W{i}']
          b = self.params[f'b{i}']
          hidden_layer_outputs[i], caches[i] = Linear_ReLU.forward(hidden_layer_outputs[i-1], W, b)

          # Dropout if enabled
          if self.use_dropout:
            hidden_layer_outputs[i], dropout_cache = Dropout.forward(hidden_layer_outputs[i], self.dropout_param)
            caches[f'dropout_cache_{i}'] = dropout_cache

        # Last layer
        W = self.params[f'W{self.num_layers}']
        b = self.params[f'b{self.num_layers}']
        scores, caches[self.num_layers] = Linear.forward(hidden_layer_outputs[self.num_layers-1], W, b)
        ...
        dh[self.num_layers], grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, caches[self.num_layers])

        for i in reversed(range(1, self.num_layers)):

          # Dropout backward
          if self.use_dropout:
            dh[i+1] = Dropout.backward(dh[i+1], caches[f'dropout_cache_{i}'])
          # finally works 

          dh[i], grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dh[i+1], caches[i])
        ...
        for i in range(1, self.num_layers + 1):
          sum_weights_squared += torch.sum(self.params[f'W{i}'] * self.params[f'W{i}'])
          grads[f'W{i}'] += reg * self.params[f'W{i}']

        loss +=  0.5 * reg * sum_weights_squared
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def get_three_layer_network_params():
    ...
    ###############################################################
    weight_scale = 1.2e-1   # Experiment with this!
...
def adam(w, dw, config=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Refrence : https://cs231n.github.io/neural-networks-3/#sgd
    learning_rate = config['learning_rate']
    beta1 = config['beta1']
    beta2 = config['beta2']
    epsilon = config['epsilon']
    ...
    m_bias_corrected = m / (1 - beta1**t)
    v_bias_corrected = v / (1 - beta2**t)
    next_w = w - learning_rate * (m_bias_corrected / (torch.sqrt(v_bias_corrected) + epsilon))
